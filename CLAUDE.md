...
**[PRUEBA TEMPORAL, sin publicar] 4 propuestas de "Chía habla" durante la lectura en voz alta** (Abel pidió mockups, luego pidió construir las 4 de verdad con un selector para comparar en vivo — 2026-09-21). Configuración > Chía tiene un campo nuevo marcado "[Prueba]" (`varianteCompaneraChiaPrueba`: ninguna/1/2/3/4) que pone `data-variante-chia` en #modo-lectura; el CSS muestra solo la variante elegida. Las 4: 1) tarjeta flotante arriba-derecha del panel, 2) burbuja circular abajo-derecha, 3) la cara de Chía reemplaza el hueco inicial del reproductor de voz mismo, 4) franja lateral fija en todo el alto del panel (verificado por coordenadas, no solo por captura, que queda pegada al borde real del panel — un capturePage anterior parecía mostrarla separada y era una lectura visual equivocada mía, no un bug). La boca (`pintarBocaCompaneraChia` en renderer.js) es un ciclo simple de 4 cuadros cada 220ms mientras `LectorVoz.estado === "leyendo"`, sin sincronía con sílabas (eso queda pendiente, es la parte que Abel dijo "a futuro"); usa los OJOS de la cara elegida en Configuración > Chía para que combine. Todo el bloque está marcado "PRUEBA TEMPORAL" en los 4 archivos (index.html, styles.css, renderer.js, configuracion.js) para borrarlo entero en cuanto Abel elija una — no mezclar con limpieza de otra cosa.

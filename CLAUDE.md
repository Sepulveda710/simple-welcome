...
**Compañera de Chía en la lectura en voz alta — decidido y construido de verdad** (2026-09-21, tras comparar las 4 propuestas con Abel). Ganó la 1 (tarjeta con cara + barras de sonido) pero reubicada: ya no flota encima del artículo, ahora es `#companera-chia`, un hijo más de `#modo-lectura` (que es `display:flex`) — el mismo trato que la barra lateral, así que aparece/desaparece del flujo real (`panel-articulo` se angosta solo para hacerle lugar, sin overlap) en vez de superponerse. Tamaño elegible en Configuración > Chía (`companeraChiaTamano`: pequena/mediana/grande, selector real, ya no de prueba) vía `data-tamano` + reglas CSS `.companera-chia[data-tamano="..."]`. Se muestra/oculta junto con el reproductor de voz (mismo `activo` que ya se usaba ahí). La boca (`pintarBocaCompaneraChia`) sigue siendo un ciclo simple de 4 cuadros cada 220ms mientras `LectorVoz.estado === "leyendo"`, sin sincronía con sílabas — eso sigue pendiente, es lo que Abel llamó "a futuro". Las variantes 2/3/4 y el selector de prueba se borraron enteros. Verificado con geometría real (no solo captura): el panel queda separado del artículo por el gap de 16px normal del flex, sin overlap, y el tamaño cambia en caliente desde Configuración.
